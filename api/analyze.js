...
- 한자어·행정용어·전문용어·외국어는 반드시 일상어로 바꿈
  (예: "납부하다"→"돈을 내다", "과태료"→"벌금")
- 어르신을 부를 때 '할머니'·'할아버지'처럼 성별을 정하는 호칭은 쓰지 마세요.
  '어르신'이라고 하거나 호칭 없이 바로 설명하세요. (음성안내·쉬운설명 모두 해당)

["음성안내"는 어떻게 쓸지]
- 화면 항목을 그대로 읽는 게 아니라, 무슨 서류인지 → 뭘 하면 되는지 → 조심할 점을
  다정한 손주가 옆에서 말해주듯 자연스럽게 이어서 설명하는 3~5문장.
- 번호("1번"), 기호, 목록 없이 자연스러운 구어체 존댓말. 금액·날짜는 말로 풀어서
  (예: "342,510원" → "34만 2천 원 정도", "12.15" → "12월 15일까지").
- 위험한 서류면 놀라지 않게 차분히, 하지만 분명하게 하지 말라고 안내.
...
[출력 형식]
{
 ...
 "한줄요약": "25자 이내",
 "쉬운설명": "3~4문장",
 "음성안내": "다정한 손주가 어르신 옆에서 말해주듯 자연스럽게 풀어 설명하는 3~5문장. 번호·기호 없이 이어지는 말투로.",
...
- 발신처가 분명한 정상적인 기관·기업의 안내(예: 자동차 리콜·안전 안내, 관공서 고지서,
  은행 명세서, 제품 안내)는 인터넷 주소·QR·전화번호가 들어 있어도, 위의 사칭·미끼·
  개인정보 요구·겁박 신호가 없으면 "안전"으로 정하세요.
- "주의"는 발신처가 불분명하거나 진위 확인이 필요한 애매한 경우에만 씁니다.
- 같은 서류는 늘 같은 위험도가 나오도록, 위 기준을 흔들림 없이 일관되게 적용하세요.
...
const OPENAI_SCHEMA = {
  ...
  schema: {
    ...
    required: ['문서분류', '문서종류', '한줄요약', '쉬운설명', '음성안내', '핵심주의', '할일', '낱말풀이', '위험도', '위험이유', 'URL목록',
...
function parseAndValidate(text) {
  ...
  if (typeof obj.음성안내 !== 'string') obj.음성안내 = '';
  return obj;
}
  ...
  const res = await fetch(
    'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent',
    {
      ...
      body: JSON.stringify({
        ...
        generationConfig: {
          temperature: 0,   // 폴백(Gemini) 결과도 매번 같도록 결정성 최대화
